...
type
    ...
    TgoOpenSSLData = procedure(const ABuffer: Pointer; const ASize: Integer)
      of object;
    ...
    public
        ...
        property OnConnected: TgoOpenSSLNotify read FOnConnected
          write FOnConnected;
    ...
    public
        ...
        class procedure SetCertificate(ctx: PSSL_CTX;
          const ACertificate, APrivateKey: TBytes;
          const APassword: UnicodeString = ''); overload;
        class procedure SetCertificate(ctx: PSSL_CTX;
          const ACertificateFile, APrivateKeyFile: UnicodeString;
          const APassword: UnicodeString = ''); overload;
    public
        class function Sign_RSASHA256(const AData: TBytes;
          const APrivateKey: TBytes; out ASignature: TBytes): Boolean;
...
begin
    ...
    begin
        ...
        begin
            { force TLS 1.2 }
            SetSSLCTXOptions(FSSLContext, SSL_OP_ALL + SSL_OP_NO_SSLv2 +
        ...
        begin
            if FPrivateKey = nil then
                TgoSSLHelper.SetCertificate(FSSLContext, FCertificate,
                  FCertificate, FPassword)
            else
                TgoSSLHelper.SetCertificate(FSSLContext, FCertificate,
                  FPrivateKey, FPassword);
...
function TgoOpenSSL.Write(const ABuffer: Pointer; const ASize: Integer)
  : Boolean;
var
    Bytes: Integer;
    Error: Integer;
...
class procedure TgoSSLHelper.SetCertificate(ctx: PSSL_CTX;
  const ACertificate, APrivateKey: TBytes; const APassword: UnicodeString = '');
var
...
begin
    ...
    begin
        ...
        PrivateKey := PEM_read_bio_PrivateKey(BIOPrivateKey, nil, nil,
          @Password[1]);
    end
    else
...
class function TgoSSLHelper.Sign_RSASHA256(const AData: TBytes;
  const APrivateKey: TBytes; out ASignature: TBytes): Boolean;
var
    BIOPrivateKey: PBIO;
    PrivateKey: PEVP_PKEY;
...
class function TgoSSLHelper.HMAC_SHA256(const AKey,
  AData: RawByteString): String;
const
    EVP_MAX_MD_SIZE = 64;
var
...
begin
    ...
    MessageAuthCode := HMAC(EVP_sha256, @AKey[1], Length(AKey), @AData[1],
      Length(AData), @Buffer[0], Size);
...
begin
    ...
    MessageAuthCode := HMAC(EVP_sha1, @AKey[1], Length(AKey), @AData[1],
      Length(AData), @Result[0], Size);
    if MessageAuthCode <> nil then
        SetLength(Result, Size);
end;
