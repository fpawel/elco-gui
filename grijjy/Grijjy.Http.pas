...
type
    { Http events }
    TOnRedirect = procedure(Sender: TObject; var ALocation: String;
      const AFollow: Boolean) of object;
    TOnPassword = procedure(Sender: TObject; var AAgain: Boolean) of object;
    TOnRecv = procedure(Sender: TObject; const ABuffer: Pointer;
      const ASize: Integer; var ACreateResponse: Boolean) of object;
    TOnSent = procedure(Sender: TObject; const ABuffer: Pointer;
...
type
    ...
    public
        ...
        function Read(out ABytes: TBytes; const ACount: Integer)
          : Boolean; overload;

        { Read up to length number of bytes from the buffer, for nghttp2 }
        function Read(const ABuffer: Pointer; var ALength: NativeInt)
          : Boolean; overload;

        { Read all bytes up to and including substring match }
        function ReadTo(const ASubStr: RawByteString;
...
type
    ...
    TgoHttpHeader = record
        ...
        Value: String;
{$IFDEF HTTP2}
        NameAsISO8859: ISO8859String;
        { nghttp2 requires a pointer to a memory object we maintain }
...
{$IFDEF HTTP2}
        ...
        function nghttp2_on_stream_close_callback(session: pnghttp2_session;
          stream_id: int32; error_code: uint32; user_data: Pointer)
          : Integer; cdecl;
...
{$ENDIF}
    ...
    private
        ...
        function DoRequest(const AMethod, AURL: String; out AResponse: TBytes;
    ...
    public
        constructor Create(const AHttp2: Boolean = False;
          const ABlocking: Boolean = True);
        destructor Destroy; override;
    public
        { Get method }
    ...
    private
        ...
        FResponse: TBytes;
    public
        constructor Create(const AHttpClient: TgoHttpClient;
          const AResponseHeaders: TgoHttpHeaders;
          const AResponseStatusCode: Integer;
          const AResponseContentType: String;
          const AResponseContentCharset: String; const AResponse: TBytes);
    public
...
function data_source_read_callback(session: pnghttp2_session; stream_id: int32;
  buf: puint8; len: size_t; data_flags: puint32; source: pnghttp2_data_source;
  user_data: Pointer): ssize_t; cdecl;
var
    Http: TgoHttpClient;
begin
    Assert(Assigned(user_data));
    Http := TgoHttpClient(user_data);
    Result := Http.nghttp2_data_source_read_callback(session, stream_id, buf,
      len, data_flags, source, user_data);
end;

function on_header_callback(session: pnghttp2_session;
  const frame: pnghttp2_frame; const Name: puint8; namelen: size_t;
  const Value: puint8; valuelen: size_t; flags: uint8; user_data: Pointer)
  : Integer; cdecl;
var
    Http: TgoHttpClient;
begin
    Assert(Assigned(user_data));
    Http := TgoHttpClient(user_data);
    Result := Http.nghttp2_on_header_callback(session, frame, name, namelen,
...
function on_data_chunk_recv_callback(session: pnghttp2_session; flags: uint8;
  stream_id: int32; const data: puint8; len: size_t; user_data: Pointer)
  : Integer; cdecl;
var
    Http: TgoHttpClient;
begin
    Assert(Assigned(user_data));
    Http := TgoHttpClient(user_data);
    Result := Http.nghttp2_on_data_chunk_recv_callback(session, flags,
      stream_id, data, len, user_data);
end;

function on_stream_close_callback(session: pnghttp2_session; stream_id: int32;
...
begin
    Assert(Assigned(user_data));
    Http := TgoHttpClient(user_data);
    Result := Http.nghttp2_on_stream_close_callback(session, stream_id,
...
function TThreadSafeBuffer.Read(out ABytes: TBytes;
...
function TThreadSafeBuffer.ReadTo(const ASubStr: RawByteString;
  out ABytes: TBytes): Boolean;
var
    Size, Index: Integer;
begin
        ...
        if FSize > Size then
            for Index := 0 to FSize - Size do
                ...
                begin
                    ...
                    begin
                        Move(FBuffer[Index + Size], FBuffer[0],
                          FSize - (Index + Size));
...
constructor TgoHttpResponseMessage.Create(const AHttpClient: TgoHttpClient;
  const AResponseHeaders: TgoHttpHeaders; const AResponseStatusCode: Integer;
...
begin
    ...
    try
        ...
        begin
            if (AForce) or (HttpClient.IdleTime > HttpClient.FRecvTimeout) then
                if (HttpClient.State = TgoHttpClientState.Sending) or
                  (HttpClient.State = TgoHttpClientState.Receiving) then
                    HttpClient.Close;
            if (AForce) or (HttpClient.IdleTime > HttpClient.FRecvTimeout) or
...
constructor TgoHttpClient.Create(const AHttp2: Boolean = False;
  const ABlocking: Boolean = True);
{$IFDEF HTTP2}
var
...
{$IFDEF HTTP2}
    ...
    begin
        nghttp2_session_callbacks_set_on_header_callback(FCallbacks_http2,
          on_header_callback);
        nghttp2_session_callbacks_set_on_frame_recv_callback(FCallbacks_http2,
          on_frame_recv_callback);
        nghttp2_session_callbacks_set_on_data_chunk_recv_callback
          (FCallbacks_http2, on_data_chunk_recv_callback);
        nghttp2_session_callbacks_set_on_stream_close_callback(FCallbacks_http2,
          on_stream_close_callback);
        if (nghttp2_session_client_new(FSession_http2, FCallbacks_http2,
          Self) = 0) then
        begin
            Settings.settings_id := NGHTTP2_SETTINGS_MAX_CONCURRENT_STREAMS;
            Settings.Value := 100;
            Error := nghttp2_submit_settings(FSession_http2, NGHTTP2_FLAG_NONE,
              @Settings, 1);
            if (Error <> 0) then
...
function TgoHttpClient.nghttp2_data_source_read_callback
  (session: pnghttp2_session; stream_id: int32; buf: puint8; len: size_t;
  data_flags: puint32; source: pnghttp2_data_source;
  user_data: Pointer): ssize_t;
begin
...
function TgoHttpClient.nghttp2_on_header_callback(session: pnghttp2_session;
  const frame: pnghttp2_frame; const Name: puint8; namelen: size_t;
  const Value: puint8; valuelen: size_t; flags: uint8;
...
{$ENDIF}
            ...
            begin
                ...
                if Index >= 0 then
                    FResponseContentCharset := FResponseContentType.Substring
                      (Index + Length(S_CHARSET) + 1,
                      Length(FResponseContentType) - Index -
                      Length(S_CHARSET)).ToLower;
            end;
...
function TgoHttpClient.nghttp2_on_data_chunk_recv_callback
  (session: pnghttp2_session; flags: uint8; stream_id: int32;
...
function TgoHttpClient.nghttp2_on_stream_close_callback
  (session: pnghttp2_session; stream_id: int32; error_code: uint32;
  user_data: Pointer): Integer;
begin
    if stream_id = FStreamId2 then
    begin
...
{$IFDEF HTTP2}
    ...
    begin
        ...
        begin
            ...
            FInternalHeaders.AddOrSet('authorization',
              'Basic ' + TEncoding.Utf8.GetString
              (goBase64Encode(TEncoding.Utf8.GetBytes(_Username + ':' +
              _Password))));
        end
        else if (FAuthorization <> '') then
...
{$ENDIF}
    begin
        ...
        FRequestStatusLine := FMethod.ToUpper + ' ' + Path + ' ' + 'HTTP/' +
          FHttpVersion;
        ...
        if (FRequestBody <> '') then
            FInternalHeaders.AddOrSet('Content-Length',
              Length(FRequestBody).ToString)
        else if (FRequestData <> nil) then
            FInternalHeaders.AddOrSet('Content-Length',
              Length(FRequestData).ToString)
        else
...
{$IFDEF HTTP2}
            ...
            begin
                ...
                { submit request }
                FStreamId2 := nghttp2_submit_request(FSession_http2, Nil,
                  @FHeaders2[0], Length(FHeaders2), @DataProvider, Self);
...
{$ENDIF}
            begin
                Headers := FRequestStatusLine + CRLF + FInternalHeaders.AsString
                  + FRequestHeaders.AsString + CRLF;
{$IFDEF LOGGING}
                // grLog('RequestHeader', TEncoding.ASCII.GetBytes(Headers));
{$ENDIF}
                if (FRequestBody <> '') then
                    Result := FConnection.Send(TEncoding.ASCII.GetBytes(Headers)
                      + TEncoding.Utf8.GetBytes(FRequestBody))
                else
                    Result := FConnection.Send(TEncoding.ASCII.GetBytes(Headers)
                      + FRequestData);
            end;
...
begin
    ...
    case FResponseStatusCode of
        301, 302, 303, 307, 308, 808: { redirect? }
            begin
...
{$ENDIF}
        ...
        try
            if (FConnection <> nil) and
              ...
              (FURI.Host <> FLastURI.Host) or (FURI.Port <> FLastURI.Port)) then
            begin
            ...
            if FConnection = nil then
            begin
                FConnection := _HttpClientSocketManager.Request(FURI.Host,
                  FURI.Port);
...
var
    ...
    begin
        ...
        try
            Headers.Text := TEncoding.ASCII.GetString(ABytes, 0,
              Length(ABytes) - 4); // exclude CRLFCRLF

            if Headers.Count > 0 then
            begin
                { response status code }
                Strings := Headers[0].ToLower.Substring
                  (Headers[0].ToLower.LastIndexOf('http:/')).Split([#32]);
                ...
                FContentLength :=
                  StrToInt64Def(FResponseHeaders.Value(S_CONTENT_LENGTH), -1);
                if FContentLength < 0 then
                    { chunked encoding }
                    FTransferEncoding :=
                      FResponseHeaders.Value(S_TRANSFER_ENCODING);
            end;
        finally
            Headers.Free;
        end;
                    ...
                    begin
                        ...
                        if FChunkSize = -1 then
                            ...
                            begin
                                ...
                                Index := String(S).IndexOf(';');
                                { skip optional chunk parameters }
                                if Index >= 0 then
                                    SetLength(S, Index);
                                FChunkSize :=
                                  StrToInt64Def('$' + String(S), -1);
{$IFDEF LOGGING}
...
{$ENDIF}
                        ...
                        else if FChunkSize = 0 then // completed
                        begin
...
procedure TgoHttpClient.OnSocketRecv(const ABuffer: Pointer;
  const ASize: Integer);
var
...
{$ENDIF}
    ...
    begin
        ...
        begin
            if ResponseHeader and ResponseContent then
            begin
                ResponseMessage := TgoHttpResponseMessage.Create(Self,
                  FResponseHeaders, FResponseStatusCode, FResponseContentType,
                  FResponseContentCharset, FResponse);
...
function TgoHttpClient.BytesToString(const ABytes: TBytes;
  const ACharset: String): String;
begin
    if ACharset = 'iso-8859-1' then
        Result := TEncoding.ANSI.GetString(ABytes)
    else if ACharset = 'utf-8' then
...
function TgoHttpClient.Get(const AURL: String;
  const AConnectTimeout, ARecvTimeout: Integer): String;
var
    Response: TBytes;
begin
    if Get(AURL, Response, AConnectTimeout, ARecvTimeout) then
...
function TgoHttpClient.Head(const AURL: String;
  const AConnectTimeout, ARecvTimeout: Integer): Boolean;
var
    AResponse: TBytes;
begin
...
function TgoHttpClient.Post(const AURL: String;
  const AConnectTimeout, ARecvTimeout: Integer): String;
var
    Response: TBytes;
begin
...
function TgoHttpClient.Put(const AURL: String;
  const AConnectTimeout, ARecvTimeout: Integer): String;
var
    Response: TBytes;
begin
...
function TgoHttpClient.Delete(const AURL: String; out AResponse: TBytes;
  const AConnectTimeout, ARecvTimeout: Integer): Boolean;
begin
    Result := DoRequest('DELETE', AURL, AResponse, AConnectTimeout,
      ARecvTimeout);
end;
...
function TgoHttpClient.Delete(const AURL: String;
  const AConnectTimeout, ARecvTimeout: Integer): String;
var
    Response: TBytes;
begin
...
function TgoHttpClient.Options(const AURL: String; out AResponse: TBytes;
  const AConnectTimeout, ARecvTimeout: Integer): Boolean;
begin
    Result := DoRequest('OPTIONS', AURL, AResponse, AConnectTimeout,
      ARecvTimeout);
end;
...
function TgoHttpClient.Options(const AURL: String;
  const AConnectTimeout, ARecvTimeout: Integer): String;
var
    Response: TBytes;
begin
